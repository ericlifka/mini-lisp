...
export function parseString(string) {
    ...
    const shouldCloseString = (ch) =>
        state === STATE.inString && ch === '"' && !escapeFlag
    const shouldContinueString = (ch) => state === STATE.inString

    const shouldOpenToken = (ch) =>
        state === STATE.ready && symbolChars.indexOf(ch) !== -1
    const shouldCloseToken = (ch) =>
        state === STATE.inToken &&
        (/\s/.test(ch) || syntaxCloseChars.indexOf(ch) !== -1)
